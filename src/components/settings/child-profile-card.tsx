...
import { LEARNING_STYLE_OPTIONS } from '@/types/settings'
...
export function ChildProfileCard({ child, onEdit, onDuplicate, onDelete }: ChildProfileCardProps) {
  const [deleteOpen, setDeleteOpen] = useState(false)
  const [deleteLoading, setDeleteLoading] = useState(false)

  const initials = child.name ? child.name.charAt(0).toUpperCase() : '?'
  const learningStyleLabel =
    (LEARNING_STYLE_OPTIONS ?? []).find((o) => o.value === child.learningStyle)?.label ?? child.learningStyle
